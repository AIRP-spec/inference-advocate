...
## Never full-regenerate over the wrong base or with derived-only fixes

The discarded 8208 generation attempt (September 2026) taught four lessons that only a wasted run can teach.

**Lesson 8208a: Never generate a corpus over a public shared proxy.** The writer model was served from a third-party endpoint with no rate assurance and no guaranteed availability. The run stalled, restarted, and took hours. Worse, the proxy introduced non-determinism: the same (seed, prompt, slot) could yield different output across retries, defeating reproducibility. A corpus must be reproducible from its recipe and seed. Use a local model or a dedicated API key on a stable endpoint. Do not generate training data through a shared public gateway.

**Lesson 8208b: The recipe and the corpus must stay in register.** The 8208 attempt started from the wrong base: 8116 rows (including the 110 flag-disclaim composed negatives) instead of the intended 7914-row round-3 base. The recipe called for stripping flag-disclaim, but generation ran before the recipe was corrected. The result was a corpus that corresponded to no committed recipe, which is the one thing reproducibility cannot tolerate. A corpus is an artifact of a recipe. Generate only after the recipe is committed and verified with `--plan`.

**Lesson 8208c: Recipe fixes belong in the recipe, not in derived corpus files.** The 26 multi-label relabels (dropping criminal_assistance from violence×CA duals that were wrong under the method test) were hand-edited into corpus.jsonl after an earlier generation, rather than being fixed in the recipe scaffolds themselves. When 8208 regenerated, those fixes were undone. A hand-edit to a derived file is not a fix; it is a time bomb. If the labels are wrong, fix the scaffold or the slot spec. If the content is wrong, fix the scaffold. Do not patch the output and call it done.

**Lesson 8208d: Do not align scaffolds to a frozen heuristic; fix the heuristic.** The method test heuristic v1 preferred `then`+verb-bank templates and `Instruction:` frames. The `violence-named-act-vs-method` scaffold achieved 24 of 24 compliance with v1, which looked like success. It was not. The scaffold was teaching regex compliance, not operational method depth. When the correct heuristic (v2, ordinary operational English) was written, 20 of 109 wrong duals flipped to pass without any content changes. The lesson: if your scaffolds are contorting to satisfy a regex, the regex is probably wrong. Fix the heuristic to recognize real method language, then improve the scaffolds to teach that, not template shapes. The heuristic should describe the boundary; the scaffolds should teach across it. Never the reverse.

**Lesson 8208e: `--replace-kinds` is composed-only; document writer-path limitations separately.** The `violence-method` writer slot (40 rows) cannot be scoped-replaced because `generate.mjs --replace-kinds` is implemented only for composed scaffolds, not for writer-generated slots. Fixing that would require extending the generator to cache per-slot output and selectively regenerate, which is not trivial. The 7914-base redo documents this as a known outstanding defect in LESSONS.md and in the recipe's own note field, rather than building the feature mid-round. A limitation you can name is better than a half-built feature that fails silently. If a slot cannot be replaced, say so in the recipe and move on.
